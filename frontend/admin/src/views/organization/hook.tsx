...
import type { IndexParams } from "@/api/organization";
import * as organizationApi from "@/api/organization";
import { message } from "@shared/utils";
import { convertDateRangeToISO } from "@/views/system/utils";

export function useOrganization(tableRef) {
  ...
  const handleDestroy = (id: number) => {
    organizationApi.destroy(id).then(() => {
      message("删除成功", {
        type: "success"
      });
      onSearch();
  ...
  const handleBatchDestroy = (ids: number[]) => {
    organizationApi.batchDestroy(ids).then(() => {
      message("删除成功", {
        type: "success"
      });
      onSearch();
