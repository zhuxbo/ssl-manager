        ...
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            <el-form-item
              ...
              prop="plus"
            >
              <el-select v-model="formData.plus">
...
// 是否显示赠送时间选项
const showPlusOption = computed(() => {
  return (
    ["apply", "batchApply"].includes(props.actionType) &&
    ["certum", "positive", "sectigo", "ssltrus"].includes(
      formData.product?.brand?.toString().toLowerCase() || ""
    )
  );
});
...
const loadOrderInfo = (id: number) => {
  show(id).then(({ data }) => {
    ...
    disabledFields.value = ["user_id", "product_id"];
