...
export function useFunds(tableRef) {
  ...
  const handleBatchDestroy = (ids: number[]) => {
    fundsApi.batchDestroy(ids).then(() => {
      message("删除成功", {
        type: "success"
      });
      onSearch();
  ...
  const handleRefunds = (id: number) => {
    fundsApi.refunds(id).then(() => {
      message("退款成功", {
        type: "success"
      });
      onSearch();
