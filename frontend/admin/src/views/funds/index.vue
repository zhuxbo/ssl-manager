...
import { fundTypeOptions } from "./dictionary";
...
<template>
  <div class="main">
    ...
    <PureTableBar title="资金管理" :columns="tableColumns" @refresh="onSearch">
      <template #buttons>
        <el-button type="success" @click="openStoreByType('addfunds')">
          充值
        </el-button>
        <el-button type="primary" @click="openStoreByType('deduct')">
          扣款
        </el-button>
      </template>
        ...
        >
          <template #operation="{ row }">
            <el-button
              v-if="
                row.status === 0 &&
                row.pay_method !== 'alipay' &&
                row.pay_method !== 'wechat'
              "
            ...
            <el-button
              v-if="
                row.status === 0 &&
                row.type === 'addfunds' &&
                (row.pay_method === 'alipay' || row.pay_method === 'wechat')
              "
              class="reset-margin !outline-none"
              type="primary"
              link
            ...
            <el-popconfirm
              v-if="row.status === 1 && row.type === 'addfunds'"
              title="确定要退款吗？"
              width="160px"
              @confirm="handleRefunds(row.id)"
            >
              <template #reference>
                <el-button
                  class="reset-margin !outline-none"
            ...
            <el-popconfirm
              v-if="row.status === 1 && row.type === 'deduct'"
              title="确定要退回吗？"
              width="160px"
            ...
            <el-popconfirm
              v-if="
                row.status === 0 &&
                dayjs(row.created_at).isBefore(dayjs().subtract(2, 'hour'))
              "
              title="确定要删除吗？"
              width="160px"
    ...
    <PlusDrawerForm
      ...
      :title="
        fundTypeOptions.find(item => item.value === storeValues.type)?.label
      "
      confirmText="提交"
      cancelText="取消"
