...
const columns = [
  ...
  { label: "请求IP", prop: "ip" },
  {
    label: "状态",
    prop: "status",
    cellRenderer: () => (props.data.status === 1 ? "成功" : "失败")
  },
  {
    label: "请求时间",
    prop: "created_at",
...
const dataList = ref([
  { title: "响应", name: "response", data: props.data.response },
  { title: "参数", name: "params", data: props.data.params }
]);
</script>
...
<template>
  <div>
    <el-scrollbar>
      <PureDescriptions border :data="[data]" :columns="columns" :column="3" />
    </el-scrollbar>
