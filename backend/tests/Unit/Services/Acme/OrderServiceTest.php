...
/**
 * 创建产品价格（OrderService 测试用）
 */
function createOrderServiceProductPrice(int $productId, $user): void
{
    ProductPrice::create([
...
test('create validates san count against product limits', function () {
    ...
    expect($result['error'])->toBe('rejectedIdentifier');
});
...
test('create charges additional san fee when exceeding purchased', function () {
    ...
    expect($newCert->standard_count)->toBe(3);
});
...
test('create does not add gift root domain', function () {
    ...
    expect($resultCert->alternative_names)->toBe('www.example.com');
});
...
test('create stores api id in cert', function () {
    ...
    expect($result)->toHaveKey('order');
    expect((int) $result['order']->api_id)->toBe(12345);
});
...
test('get acme status derives correctly', function () {
    ...
    expect($this->service->getAcmeStatus($cert5))->toBe('pending');
});
...
test('create prefers order id over user id lookup', function () {
    ...
    expect($result['order']->order_id)->toBe($order2->id);
});
...
test('get returns null for non acme cert', function () {
    $user = $this->createTestUser();
    $product = $this->createTestProduct();
    $order = $this->createTestOrder($user, $product);
    $cert = $this->createTestCert($order, [
